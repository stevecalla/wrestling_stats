...
async function get_teams_from_multi_team_event(
  ...
  try {
    ...
    // Teams are inside #teamsFrame inside #eventTeamFrame
    const teams_frame_locator = modal_locator.locator("#teamsFrame");
    const team_links_locator = teams_frame_locator.locator(
      'a[target="_blank"][href*="EventDuals.jsp"]'
    );
      ...
      if (has_at) {
        ...
        for (const t of teams) {
          out.push({
            ...
            team_id: null, // filled for multi-team only
...
async function main(
  ...
  // TODO: parameterize these if needed
  const OVERALL_START = make_date(2025, 11, 1);
  const OVERALL_END = make_date(2026, 3, 31);
    ...
    while (local_page_index < matches_page_limit) {
      ...
      const raw_rows = await target_frame.evaluate(
        schedule_extractor_source(),
        {
          ...
          base_span_row_counter: rows_seen_in_span,
        }
      );
  ...
  // ---------- MAIN DATE-SPAN LOOP ----------
  console.log(
    `📆 Running date-span loop from 2025-11-01 to 2026-03-31 in ${SPAN_DAYS}-day chunks`
  );
  ...
  for (
    ...
    const span_start = new Date(cursor.getTime());
    const span_end = new Date(cursor.getTime());
    span_end.setDate(span_end.getDate() + SPAN_DAYS - 1);
    if (span_end > OVERALL_END) span_end.setTime(OVERALL_END.getTime());

    const span_label = format_span_label(span_start, span_end);
    console.log(
      `\n==================== DATE SPAN #${span_index}: ${span_label} ====================`
    );
    ...
    try {
      ...
      await set_date_inputs(results_frame_for_search, span_start, span_end);
