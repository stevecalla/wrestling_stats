...
} from "./step_3_get_wrestler_match_history_parallel_scrape_v4/step_1_create_scrape_tasks_table.js";
import {
  step_2_seed_tasks,
  requeue_locked_failed_for_task_set,
} from "./step_3_get_wrestler_match_history_parallel_scrape_v4/step_2_insert_seed_tasks.js";
...
export async function main(
  ...
  // ✅ NEW: tune claim_batch_size in is_extended_failure_retry (less aggressive / less lock contention)
  const claim_batch_size = is_extended_failure_retry ? 15: 7;
  ...
  const results = await Promise.allSettled(
    ports_to_use.map((port, idx) =>
      (async () => {
        ...
        const { browser, page, context } = await step_0_launch_chrome_developer_v3(
          url_home_page,
          port
        );
        ...
        await step_3_get_match_history_worker_v4({
          ...
          is_extended_failure_retry, // ✅ NEW: passed to worker
        });
